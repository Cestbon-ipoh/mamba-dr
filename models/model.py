...
from models.mamba import MambaFusion, MambaConfig, MambaBlock, SelectiveModule, GlobalMaskModule
...
class SMVDR(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
                 embed_dim=768, depth=6, num_heads=6, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., num_views=2):
        print('new-model! ')
        # Transformer
        super().__init__()
        ...
        # Global Mask
        self.global_mask = GlobalMaskModule(
            in_channels=embed_dim * num_views,
            version=1,
        )
        ...
        self.trans_cls_head_dan = nn.Sequential(
    ...
    def joint(self, x):
        ...
        b, v, c, h, w = x.shape
        arr = torch.zeros((b, c, h, 2 * w), device=x.device, dtype=x.dtype)
        arr[:, :, :, :w] = x[:, 0, :, :, :]  # 左
        arr[:, :, :, w:] = x[:, 1, :, :, :]  # 右
        return arr
        #arr = x
    ...
    def forward(self, x, labels=None):
        ...
        mask = x[:, 3:4, :, :].float() / 255.0
        x_base = self.maxpool(self.act1(self.bn1(self.conv1(x))))
        mask_resized = torch.nn.functional.interpolate(mask, size=x_base.shape[-2:], mode='bilinear', align_corners=False)
        #用mask加权特征
        x_base = x_base * (1.0 + mask_resized)
        ...
        mv_x_y = rearrange(mv_x_y, 'b c (v e) -> b v c e', v=2)
        # mambafusion
        ...
        return output,expert_probabilities, selector, diff_se
        '''拼接两个视角的cls_token
        cls_tokens = torch.cat([x_t[:, i, 0] for i in range(self.num_view)], dim=-1)
        #MLP
        output = self.trans_cls_head(cls_tokens)'''
        
        #return output,None, selector, diff_se
