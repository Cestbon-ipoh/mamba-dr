...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig, window=7):
        ...
        # auto_res
        self.auto_res_eps = nn.Parameter(torch.tensor(0.5))

        # Global Mask
        self.global_mask = GlobalMaskModule(config.d_model)
    ...
    def forward(self, x):
        ...
        eps = torch.sigmoid(self.auto_res_eps)
        output = eps * output + (1 - eps) * residual
...
class MambaFusion(nn.Module):
    ...
    def forward(self, tensors: List):
        ...
        x = torch.cat([tensor[:, i::2, :] for i in range(2) for tensor in tensors], dim=1)
        _, L, _ = x.shape
...
# GM模块
class GlobalMaskModule(nn.Module):
    def __init__(self, in_channels, version=1,):
        super(GlobalMaskModule, self).__init__()
        self.version = version
        self.norm = nn.LayerNorm(in_channels)
        self.W_R = nn.Linear(in_channels, in_channels, bias=False)
    ...
    def forward(self, x):
        # x: [B, V*N, C]
        B, VN, C = x.shape
        V = self.version if hasattr(self, 'version') and isinstance(self.version, int) and self.version > 1 else 4  
        if self.version == 1:

            cls_tokens = x[:, :V, :]  # [B, V, C]
            x_ = x[:, V:, :]          # [B, V*N', C]
        else:
            x_ = x
        ...
        R = self.W_R(x_)
        S = self.W_S(x_)
        U = torch.bmm(R, S.transpose(1, 2))
        U = self.conv(U.unsqueeze(1)).squeeze(1)
        attention = torch.softmax(U, dim=-1)
        out = torch.bmm(attention, x_)
